...
def _handle_export_zip(payload: Dict[str, Any]) -> None:
    """Mark exported assets after zip job."""
    ...
    if asset_ids:
        try:
            ...
            result = loop.run_until_complete(asset_service.mark_exported())
            loop.close()
            logger.info("Export automation: marked assets as exported after zip job")
