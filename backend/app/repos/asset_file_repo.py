...
class AssetFileRepo(BaseRepo):
    ...
    def create_for_files(
        ...
        """Insert asset file and return the created row. Uses file_url for storage path."""
        rows = self._fetch(
    ...
    def get_filename_for_blob_path(self, blob_path: str, conn=None) -> Optional[str]:
        """Return file_name for a row matching blob_path in file_url or file_path. Used for download response."""
        pat = f"%{blob_path}%"
        rows = self._fetch(
    ...
    def get_file_meta_for_blob_path(self, blob_path: str, conn=None) -> Tuple[str, str]:
        """Return (file_name, file_type) for response. Matches by file_url containing path."""
        pat = f"%{blob_path}%"
        rows = self._fetch(
