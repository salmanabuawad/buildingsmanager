...
async def create_zip_job(
    request: Request,
    body: CreateZipRequest,
    background_tasks: BackgroundTasks,
    current_user: CurrentUser = Depends(get_current_user_users_table),
):
    ...
    job_id = create_job()
    entries = [ZipEntry(path=e.path, filename_in_zip=e.filename_in_zip) for e in body.entries]
    background_tasks.add_task(_build_zip_task, job_id, body.zip_filename, entries)
