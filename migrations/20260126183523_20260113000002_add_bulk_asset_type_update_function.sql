/*
  # Bulk update asset types with distribution reset logic

  Adds a single RPC that updates multiple asset_types rows in one call while
  preserving the existing transactional logic in:
    - update_asset_type_with_distribution_reset(p_id, p_updates)

  This keeps UI "Save All" operations truly bulk (single network call) while
  still setting building distribution flags when non_accountable_for_distribution changes.
*/
...
COMMENT ON FUNCTION update_asset_types_bulk_with_distribution_reset IS 'Bulk update asset_types by calling update_asset_type_with_distribution_reset for each item, in a single transaction. Aggregates affected building_numbers.';