/*
  ...
  1. Overview
    - Updates the function to only set exported_to_automation flag
    - Removes reference to non-existent export_to_automation_at column
    
  2. Changes
    - Remove export_to_automation_at = NOW() from UPDATE statement
*/
...
BEGIN
  ...
  -- Update the assets (only set the flag, no timestamp)
  UPDATE assets
