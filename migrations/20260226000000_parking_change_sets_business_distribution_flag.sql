-- When building parking data (parking_area, shared_parking_area, number_of_parking_units) changes,
-- set need_business_distribution = true so shared parking can be re-distributed to business assets.
...
BEGIN
  ...
  LOOP
    ...
    v_old_parking_area := v_old_building.parking_area;
    ...
    IF v_updates ? 'parking_area' THEN
      v_new_parking_area := (v_updates->>'parking_area')::NUMERIC;
    ELSE
      v_new_parking_area := v_old_parking_area;
    END IF;
    ...
    v_parking_changed := (v_old_parking_area IS DISTINCT FROM v_new_parking_area)
      OR (v_old_shared_parking_area IS DISTINCT FROM v_new_shared_parking_area)
      OR (v_old_number_of_parking_units IS DISTINCT FROM v_new_number_of_parking_units);
    ...
    SET
      ...
      parking_area = CASE WHEN v_final_updates ? 'parking_area' THEN (v_final_updates->>'parking_area')::NUMERIC ELSE parking_area END,
...
COMMENT ON FUNCTION update_buildings_bulk_with_distribution_flags IS 'Bulk update buildings with distribution flags. Sets need_business_distribution when business_shared_area or parking fields (parking_area, shared_parking_area, number_of_parking_units) change.';
