  ...
  const handleExportToAutomation = useCallback(async () => {
    ...
    try {
      ...
      const result = await runExportToAutomation({
        ...
        buildingsMap: building ? new Map([[buildingNumber, building]]) : undefined,
        assetTypes: assetTypes.length > 0 ? assetTypes : await api.assetTypes.getAll(),
      ...
      if (result.failedEmails > 0) {
        ...
        successMessage += ` ⚠️ ${result.failedEmails} מיילים נכשלו${errDetail}`;
      }
      setToast({ message: successMessage, type: result.failedEmails > 0 && result.sentEmails === 0 ? 'error' : 'success' });

      setTimeout(() => setToast(null), result.failedEmails > 0 ? 12000 : 8000);
