    ...
    <div className="p-4 max-w-lg mx-auto space-y-6" dir="rtl">
      ...
      <section className="bg-white rounded-xl border border-slate-200 shadow-sm p-4">
        <h2 className="flex items-center gap-2 text-base font-medium text-slate-700 mb-3">
          <ListTodo className="h-5 w-5 text-theme-tab-active" />
          המשימות שלי
        </h2>
        {loading ? (
          ...
          <ul className="space-y-2">
            {[...tasks].reverse().map((t) => (
              <li
                key={t.id}
                className="flex flex-row-reverse items-center justify-between p-3 bg-slate-50 rounded-lg border border-slate-100 cursor-pointer hover:bg-slate-100"
                ...
                <div className="flex items-center gap-2" onClick={(e) => e.stopPropagation()}>
                  {t.status === 'new' && (
                    <button
                      ...
                      className="px-3 py-1.5 bg-theme-tab-active text-white rounded-lg text-sm disabled:opacity-50"
                    >
                    ...
                    return (
                      ...
                      >
                        <div className="flex-1 min-w-0 flex items-start gap-2">
                          <button
                            type="button"
                            className="flex-shrink-0 w-20 h-20 rounded-lg overflow-hidden bg-slate-200 flex items-center justify-center p-0 border-0 cursor-pointer focus:ring-2 focus:ring-theme-action-accent focus:ring-offset-1"
