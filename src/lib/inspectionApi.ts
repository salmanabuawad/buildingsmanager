...
async function withReportAndHistory(task: Record<string, unknown>): Promise<InspectionTask> {
  ...
  const { data: report } = await client.from('inspection_reports').select('*').eq('task_id', taskId).maybeSingle();
  let files: Array<{ id: number; file_path: string; file_name: string | null; file_type: string | null; asset_ids?: number[] }> = [];
  if (report?.id) {
    const { data: fileRows } = await client.from('inspection_report_files').select('*').eq('report_id', report.id);
...
export const inspectionTasksApi = {
  list: async (params?: { status?: string; assigned_to?: number; building_number?: number; skip?: number; limit?: number }) => {
    let q = client.from('inspection_tasks').select('*').order('created_at', { ascending: false });
  ...
  get: async (taskId: number) => {
    const { data, error } = await client.from('inspection_tasks').select('*').eq('id', taskId).single();
    if (error) throw new Error(error.message);
  ...
  create: async (body: { title?: string; building_number: number; asset_ids?: number[]; assigned_to?: number; note?: string; priority?: 'high' | 'medium' | 'low' }) => {
    ...
    const { data, error } = await client
      ...
      .single();
    if (error) throw new Error(error.message);
    await client.from('inspection_reports').insert({ task_id: data.id, report_text: null });
    return withReportAndHistory(data);
  },

  update: async (taskId: number, body: { title?: string; assigned_to?: number; status?: string; note?: string; priority?: 'high' | 'medium' | 'low' }) => {
    const { data, error } = await client.from('inspection_tasks').update(body).eq('id', taskId).select().single();
    if (error) throw new Error(error.message);
    return withReportAndHistory(data);
  },
  ...
  take: async (taskId: number) => {
    ...
    const { data: task } = await client.from('inspection_tasks').select('*').eq('id', taskId).single();
    ...
    const { data, error } = await client
      ...
      .single();
    if (error) throw new Error(error.message);
    await client.from('inspection_task_history').insert({ task_id: taskId, action: 'taken', created_by: s.user_id });
    return withReportAndHistory(data);
  },
  ...
  createOtp: async (userId: number, taskId: number) => {
    const s = getSession();
    if (!s?.user_id) throw new Error('User id required');
    const { data, error } = await client.rpc('inspector_create_otp', {
      p_user_id: userId,
      p_task_id: taskId,
      p_caller_user_id: s.user_id,
    });
...
export const inspectionReportsApi = {
  getByTask: async (taskId: number) => {
    ...
    const files = report?.id
      ? (await client.from('inspection_report_files').select('*').eq('report_id', report.id)).data ?? []
      : [];
    return { task_id: taskId, report: report ? { ...report, files } : null };
  },

  upsert: async (body: { task_id: number; report_text?: string }) => {
    const { data: existing } = await client.from('inspection_reports').select('*').eq('task_id', body.task_id).maybeSingle();
    if (existing) {
      const { data, error } = await client
  ...
  listFiles: async (reportId: number) => {
    const { data, error } = await client.from('inspection_report_files').select('*').eq('report_id', reportId);
    if (error) throw new Error(error.message);
    return data ?? [];
  },
  ...
  uploadFile: async (reportId: number, file: File, assetIds?: number[]) => {
    ...
    const { data: storageData, error: storageError } = await client.storage.from(bucket).upload(path, file, { upsert: true });
  ...
  deleteFile: async (fileId: number) => {
    const { error } = await client.from('inspection_report_files').delete().eq('id', fileId);
    if (error) throw new Error(error.message);
    return null;
  },
};
