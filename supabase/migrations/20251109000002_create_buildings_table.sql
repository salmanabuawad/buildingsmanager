/*
  ...
  1. Changes
    ...
    - Create apartments table with foreign key to buildings
  
  2. Table Structure
    - building_number (bigint, PRIMARY KEY)
    - All building fields including tax_region, elevator, shared_area, etc.
    - apartments table for building unit management
*/
...
-- Create buildings table with building_number as primary key and all fields
...
CREATE INDEX idx_buildings_building_address ON buildings(building_address);
CREATE INDEX idx_apartments_building_number ON apartments(building_number);
