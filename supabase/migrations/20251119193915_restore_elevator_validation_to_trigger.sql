/*
  # Restore Elevator Validation to Asset Trigger
  
  1. Changes
    - Restores elevator validation logic to validate_asset_before_insert trigger
    - Validation checks that asset type elevator requirements match building
...
-- Check if main asset type exists in the building's tax region
SELECT * INTO v_asset_type_record
FROM asset_types
WHERE code::TEXT = NEW.main_asset_type
AND tax_region::TEXT = v_building_tax_region;

IF NOT FOUND THEN
RAISE EXCEPTION 'VALIDATION_ERROR: סוג הנכס % לא קיים באזור המס % של הבניין (Asset type % does not exist in tax region %)', 
NEW.main_asset_type, v_building_tax_region, NEW.main_asset_type, v_building_tax_region;
END IF;
...
RAISE EXCEPTION 'VALIDATION_ERROR: סוג משנה 1 (%) לא קיים באזור המס % של הבניין (Sub asset type 1 (%) does not exist in tax region %)', 
NEW.sub_asset_type_1, v_building_tax_region, NEW.sub_asset_type_1, v_building_tax_region;
END IF;
...
RAISE EXCEPTION 'VALIDATION_ERROR: סוג משנה 2 (%) לא קיים באזור המס % של הבניין (Sub asset type 2 (%) does not exist in tax region %)', 
NEW.sub_asset_type_2, v_building_tax_region, NEW.sub_asset_type_2, v_building_tax_region;
END IF;
...
RAISE EXCEPTION 'VALIDATION_ERROR: סוג משנה 3 (%) לא קיים באזור המס % של הבניין (Sub asset type 3 (%) does not exist in tax region %)', 
NEW.sub_asset_type_3, v_building_tax_region, NEW.sub_asset_type_3, v_building_tax_region;
END IF;
...
RAISE EXCEPTION 'VALIDATION_ERROR: סוג משנה 4 (%) לא קיים באזור המס % של הבניין (Sub asset type 4 (%) does not exist in tax region %)', 
NEW.sub_asset_type_4, v_building_tax_region, NEW.sub_asset_type_4, v_building_tax_region;
END IF;
...
RAISE EXCEPTION 'VALIDATION_ERROR: סוג משנה 5 (%) לא קיים באזור המס % של הבניין (Sub asset type 5 (%) does not exist in tax region %)', 
NEW.sub_asset_type_5, v_building_tax_region, NEW.sub_asset_type_5, v_building_tax_region;
END IF;
...
RAISE EXCEPTION 'VALIDATION_ERROR: סוג משנה 6 (%) לא קיים באזור המס % של הבניין (Sub asset type 6 (%) does not exist in tax region %)', 
NEW.sub_asset_type_6, v_building_tax_region, NEW.sub_asset_type_6, v_building_tax_region;
END IF;
