/*
  # Create Asset Type Fields Configuration Table
  
  1. Changes
    - Create new table `asset_type_fields` to store field configurations
    - Each row represents a field name with three boolean flags:
      - `is_asset_level`: indicates if field is at asset level
      - `is_building_level`: indicates if field is at building level
      - `is_asset_type_validation`: indicates if field is part of asset type validation
  
  2. Table Structure
    - `id` (uuid) - Primary key
    - `field_name` (text) - Name of the field (e.g., 'asset_id', 'building_number', 'main_asset_type')
  ...
  3. Security
    - Enable RLS on asset_type_fields table
...
-- Note: Triggers will be added in the next migration to automatically update
