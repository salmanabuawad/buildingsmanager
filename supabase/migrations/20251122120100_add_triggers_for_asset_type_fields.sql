/*
  # Add Triggers to Update asset_type_fields When Asset Types Change
  
  1. Changes
    - Create trigger function to sync asset_type_fields when asset_types are inserted/updated
    - Create trigger function to handle asset_types deletion (optional cleanup)
    - Add triggers to asset_types table
  
  2. Logic
    - On INSERT/UPDATE: Ensure all fields used by asset type are in asset_type_fields
    - Fields from asset_types: elevator, single_double_family, penthouse, condo, townhouses, shelter/basement
    - These fields should be marked as is_asset_type_validation = true when used by asset types
*/
...
BEGIN
  ...
  -- If you want to implement automatic cleanup, you could:
  -- 1. Check if any other asset types use the same fields
  -- 2. Check if any assets/buildings use those fields
  -- 3. Only then remove or update is_asset_type_validation flag
