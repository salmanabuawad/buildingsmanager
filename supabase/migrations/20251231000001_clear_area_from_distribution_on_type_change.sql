-- ============================================================================
-- Migration: Clear area_from_distribution when asset type changes from business to residence
-- ============================================================================
-- This migration updates the save functions to clear area_from_distribution
-- when an asset type changes from business to residence (or vice versa).
--
-- area_from_distribution is only used for business distribution, so when
-- an asset changes from business to residence, this field should be cleared.

-- ============================================================================
-- FUNCTION: save_asset_transactional - Clear area_from_distribution on type change
-- ============================================================================
...
BEGIN
  ...
  IF FOUND THEN
    ...
    IF v_asset_type_changed AND v_old_main_asset_type IS NOT NULL AND v_new_main_asset_type IS NOT NULL THEN
      ...
      -- Check if changing from business to residence (or residence to business)
      -- In both cases, clear area_from_distribution (it's only for business)
...
COMMENT ON FUNCTION save_asset_transactional IS 'Save single asset with transactional post-save actions. Sets distribution flags when asset type, size, or tax_region changes. Clears area_from_distribution when asset type changes from business to residence (or vice versa). Only sets flags if building has relevant shared area > 0.';

-- ============================================================================
-- FUNCTION: save_assets_bulk_transactional - Clear area_from_distribution on type change
-- ============================================================================
...
--    v_business_to_residence BOOLEAN := FALSE;
--
-- 2. In the loop where existing asset is fetched (around line 950-956), after getting old values, add:
--    -- Check if changing from business to residence (or vice versa)
--    IF v_asset_type_changed AND v_old_main_asset_type IS NOT NULL AND v_new_main_asset_type IS NOT NULL THEN
--      -- Get business_residence for old and new types
--      SELECT business_residence INTO v_old_business_residence
...
--    END IF;
--
-- 3. In the UPDATE statement (around line 1150), change area_from_distribution to:
--    area_from_distribution = CASE 
--      -- If changing from business to residence (or vice versa), clear area_from_distribution
--      WHEN v_business_to_residence THEN 0
--      -- Otherwise, use provided value or keep existing
...
--    END,
--
-- These changes will ensure that area_from_distribution is cleared when asset type changes
-- from business to residence (or vice versa) in bulk operations.
