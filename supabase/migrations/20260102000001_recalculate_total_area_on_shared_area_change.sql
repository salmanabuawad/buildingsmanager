/*
  # Recalculate Total Area When Business Shared Area Changes
  
  Updates the update_buildings_bulk_with_distribution_flags function to
  automatically recalculate total_building_area when business_shared_area changes,
  unless total_building_area is explicitly provided in the updates.
*/
...
BEGIN
  ...
  LOOP
    ...
    IF v_old_business_area IS DISTINCT FROM v_new_business_area THEN
      ...
      -- If total_building_area is not explicitly provided in updates, remove it so it will be recalculated
      IF NOT (v_updates ? 'total_building_area') THEN
        v_final_updates := v_final_updates - 'total_building_area';
      END IF;
    ...
    -- Recalculate total_building_area if business_shared_area changed and total_building_area wasn't explicitly provided
    IF v_old_business_area IS DISTINCT FROM v_new_business_area AND NOT (v_updates ? 'total_building_area') THEN
...
COMMENT ON FUNCTION update_buildings_bulk_with_distribution_flags IS 'Bulk update buildings and automatically set distribution flags when shared areas (residence_shared_area or business_shared_area) change. Automatically recalculates total_building_area when business_shared_area changes (unless total_building_area is explicitly provided). Sets flags to true when shared area changes. All updates happen in a single transaction. Use this function for all building updates, even single ones.';
