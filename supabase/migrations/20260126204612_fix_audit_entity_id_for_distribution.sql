/*
  # Fix audit entity_id for distribution operations
  
  1. Overview
    - For distribution operations, store building_number in entity_id (not asset IDs)
    - This allows the UI to query audit records by building number
    
  2. Changes
    - Update save_assets_bulk_transactional to use building_number as entity_id for distribution operations
    - Keep asset IDs as entity_id for other operations
*/
...
IF p_action_type IN ('distribute_shared', 'business_distribution', 'residence_distribution') THEN
-- For distribution actions, collect all assets in the building
FOR v_asset_record IN 
SELECT asset_id, building_number, payer_id, measurement_date, main_asset_type, asset_size, tax_region,
sub_asset_type_1, sub_asset_size_1, sub_asset_type_2, sub_asset_size_2,
sub_asset_type_3, sub_asset_size_3, sub_asset_type_4, sub_asset_size_4,
...
-- For distribution operations, use building_number
-- For other operations, use comma-separated list of asset IDs
IF p_action_type IN ('business_distribution', 'residence_distribution', 'distribute_shared') THEN
