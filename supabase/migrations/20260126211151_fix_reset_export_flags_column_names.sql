/*
  # Fix reset export flags trigger to use correct column names

  1. Overview
    - The trigger was using building_id instead of building_number
    - Fix all column references to match actual assets table structure
    
  2. Changes
    - Update reset_export_flags_on_change function with correct column names
    - Use building_number instead of building_id
    - Use asset_number, asset_name instead of incorrect references
*/
...
BEGIN
  ...
  IF (
    ...
    NEW.sub_asset_type_5 IS DISTINCT FROM OLD.sub_asset_size_5 OR
