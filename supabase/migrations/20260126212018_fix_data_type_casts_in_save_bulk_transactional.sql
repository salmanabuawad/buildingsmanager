/*
  # Fix data type casts in save_assets_bulk_transactional

  1. Overview
    - Fix type casting mismatches causing "operator does not exist" errors
    - tax_region column is INTEGER but function was casting to BIGINT
    - floor column is SMALLINT but function was casting to BIGINT
    
  2. Changes
    - Cast tax_region to INTEGER instead of BIGINT
    - Cast floor to SMALLINT instead of BIGINT
    - Ensure all type casts match actual column types
*/
...
-- INSERT new asset (fixed type casts: tax_region to INTEGER, floor to SMALLINT)
INSERT INTO assets (
...
(v_asset_data->>'structure_drawing_url')::TEXT, (v_asset_data->>'floor')::SMALLINT,
...
ELSE
-- UPDATE existing asset (fixed type casts: tax_region to INTEGER, floor to SMALLINT)
...
floor = COALESCE((v_asset_data->>'floor')::SMALLINT, floor),
