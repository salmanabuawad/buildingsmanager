/*
  Migration: Fix all database functions to support boolean checkbox fields
  
  This migration updates all functions that use COALESCE with checkbox fields
  to properly handle boolean values instead of TEXT.
*/

-- ============================================================================
-- 1. Fix update_buildings_bulk_with_distribution_flags function
-- ============================================================================
-- This function is already fixed in migration 20260127000005, but we ensure
-- it's correct here as well in case the migration order is different
...
BEGIN
  ...
  LOOP
    ...
    SET
      ...
      tax_region = CASE
        WHEN v_final_updates ? 'tax_region' THEN (v_final_updates->>'tax_region')::TEXT
        ELSE tax_region
      END,
      elevator = CASE
