/*
  # Add parking fields to buildings

  parking_area: total parking area (numeric)
  shared_parking_area: shared parking area (numeric)
  number_of_parking_units: count of parking units (integer)
*/

ALTER TABLE buildings ADD COLUMN IF NOT EXISTS parking_area NUMERIC;
...
COMMENT ON COLUMN buildings.parking_area IS 'שטח חניה (Parking area)';
COMMENT ON COLUMN buildings.shared_parking_area IS 'שטח חניה משותף (Shared parking area)';
COMMENT ON COLUMN buildings.number_of_parking_units IS 'מספר יחידות חניה (Number of parking units)';

-- Update the bulk update function to set the new columns
...
COMMENT ON FUNCTION update_buildings_bulk_with_distribution_flags IS 'Bulk update buildings with distribution flags. Includes parking_area, shared_parking_area, number_of_parking_units, address and note.';
