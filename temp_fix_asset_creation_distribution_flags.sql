-- Fix: Set distribution flags when new assets are created or asset size changes
-- The issue is that save_assets_bulk_transactional only sets flags for updates, not for new assets
...
BEGIN
      ...
      LOOP
        ...
        IF v_asset_id IS NOT NULL THEN
          SELECT jsonb_build_object(
            ...
            'discount_date_to', a.discount_date_to, 'business_distribution_area', a.business_distribution_area,
  ...
  LOOP
    ...
    v_asset_found := FOUND;
    v_is_new_asset := NOT v_asset_found;
    ...
    ELSE
      IF extract_boolean_from_jsonb(v_asset_data->'is_new_measurement', false) = true THEN
        INSERT INTO assets_history (asset_id, building_number, payer_id, measurement_date, main_asset_type,
          ...
          townhouses, penthouse, structure_drawing_url, discount_type, discount_date_from,
          discount_date_to, business_distribution_area, exported_to_automation, comment, created_at, updated_at)
        SELECT asset_id, building_number, payer_id, measurement_date, main_asset_type, asset_size, tax_region,
          sub_asset_type_1, sub_asset_size_1, sub_asset_type_2, sub_asset_size_2, sub_asset_type_3, sub_asset_size_3,
          sub_asset_type_4, sub_asset_size_4, sub_asset_type_5, sub_asset_size_5, sub_asset_type_6, sub_asset_size_6,
          elevator, single_double_family, condo, townhouses, penthouse, structure_drawing_url,
          discount_type, discount_date_from, discount_date_to, business_distribution_area, exported_to_automation,
      ...
      UPDATE assets SET
        ...
        business_distribution_area = CASE WHEN v_business_to_residence THEN 0
          WHEN v_asset_data->'business_distribution_area' IS NULL THEN business_distribution_area
          ELSE COALESCE((v_asset_data->>'business_distribution_area')::NUMERIC, 0) END,
    ...
    -- Set distribution flags for new assets OR when asset size/type changes
    BEGIN
      DECLARE
        v_is_business_context BOOLEAN := FALSE;
        v_is_residence_context BOOLEAN := FALSE;
      BEGIN
        -- Determine if this is a new asset or an update with changes
        IF v_is_new_asset THEN
          -- New asset: set flag based on asset type
          ...
          -- Set flags for new assets (only if building has shared area > 0)
...
COMMENT ON FUNCTION save_assets_bulk_transactional IS 'Bulk save assets with transactional operations including validation, distribution flags, and audit logging. All operations are atomic. Supports boolean checkbox fields. Sets distribution flags for new assets and when asset size/type changes.';
